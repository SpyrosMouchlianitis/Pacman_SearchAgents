...
def cornersHeuristic(state, problem):
    ...
    distancesFromFinalStates = []

    for index, item in enumerate(cornerStatus):
        if item == False:
            distancesFromFinalStates.append(util.manhattanDistance(currentPosition, corners[index]))

    heuristic = max(distancesFromFinalStates)
...
def foodHeuristic(state, problem):
    ...
    for food in foods:
        distanceIndex = position + food
        if distanceIndex in problem.heuristicInfo:
            distance = problem.heuristicInfo[distanceIndex]
        else:
            distance = util.manhattanDistance(position, food)
            problem.heuristicInfo[distanceIndex] = distance
